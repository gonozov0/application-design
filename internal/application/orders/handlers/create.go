...
import (
	...
	"application-design/pkg/responses"
...
func (h *Handler) Create(c echo.Context) error {
	...
	if err != nil {
		if errors.Is(err, ordersDomain.ErrInvalidBooking) {
			return c.JSON(http.StatusBadRequest, responses.NewError(err.Error()))
		}
	...
	if err != nil {
		if errors.Is(err, usersDomain.ErrUserNotFound) || errors.Is(err, hotelsDomain.ErrHotelNotFound) ||
			...
			return c.JSON(http.StatusNotFound, responses.NewError(err.Error()))
		}
		if errors.Is(err, ordersDomain.ErrInvalidOrder) {
			return c.JSON(http.StatusBadRequest, responses.NewError(err.Error()))
		}
		if errors.Is(err, hotelsDomain.ErrRoomAlreadyBooked) {
			return c.JSON(http.StatusConflict, responses.NewError(err.Error()))
		}
		return echo.ErrInternalServerError
	}
